...
export default function RegisterPage() {
  ...
  const onOtpSubmit = async (data) => {
    ...
    try {
        ...
        toast.success("Account verified! Let's connect your Instagram.");
        router.push('/connect');
    } catch (err) {
        toast.error(err.message || "An unknown error occurred.");
    } finally {
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="w-full max-w-md p-8">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold">{showOtpForm ? "Verify Your Account" : "Create an Account"}</h1>
            <p className="text-gray-600">{showOtpForm ? `We've sent a code to ${registeredEmail}` : "Get started with your new account"}</p>
          </div>
          {!showOtpForm ? (
            <form onSubmit={handleRegisterSubmit(onRegisterSubmit)} className="space-y-4" noValidate>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="firstName">First Name</Label>
                  <Input id="firstName" {...registerForm("firstName")} />
                  {registerErrors.firstName && <p className="text-red-500 text-sm mt-1">{registerErrors.firstName.message}</p>}
                </div>
                <div>
                  <Label htmlFor="lastName">Last Name</Label>
                  <Input id="lastName" {...registerForm("lastName")} />
                  {registerErrors.lastName && <p className="text-red-500 text-sm mt-1">{registerErrors.lastName.message}</p>}
                </div>
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
              ...
              <div>
                ...
                {registerErrors.phoneNumber && <p className="text-red-500 text-sm mt-1">{registerErrors.phoneNumber.message}</p>}
              </div>
              <div>
                <Label htmlFor="password">Password</Label>
                <Input id="password" type="password" {...registerForm("password")} />
                {registerErrors.password && <p className="text-red-500 text-sm mt-1">{registerErrors.password.message}</p>}
              </div>
              <Button type="submit" className="w-full" disabled={isRegistering}>
                {isRegistering && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {isRegistering ? "Creating..." : "Create Account"}
              </Button>
            </form>
          ) : (
            <form onSubmit={handleOtpSubmit(onOtpSubmit)} className="space-y-4" noValidate>
              <div>
                <Label htmlFor="otp">Verification Code</Label>
                <Input id="otp" {...registerOtp("otp")} placeholder="Enter 6 digit code" />
                {otpErrors.otp && <p className="text-red-500 text-sm mt-1">{otpErrors.otp.message}</p>}
              </div>
              <Button type="submit" className="w-full" disabled={isVerifying}>
                {isVerifying && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} {isVerifying ? "Verifying..." : "Verify Email"}
              </Button>
            </form>
          )}
          <div className="mt-6 text-center">
            <p className="text-sm">Already have an account? <Link href="/auth/login" className="text-blue-600 hover:text-blue-800">Sign in</Link></p>
          </div>
        </Card>
      </div>
    </>
